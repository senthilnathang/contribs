...
class EmployeeShiftRun(models.Model):
    ...
    employee_shift_ids = fields.One2many('hr.employee.shift', 'shift_run_id', string='Employee', ondelete='cascade', index=True, copy=False, readonly=True)
...
class HREmployeeShift(models.Model):
    ...
    def schedule_shift(self):
        for employee in self.env['hr.employee'].search([('active','=',True)]):
            over_lap1 = self.env['hr.employee.shift'].search([('employee_id', '=', employee.id), ('start_date', '<=', fields.Datetime.now()), ('end_date', '>=', fields.Datetime.now())])
